...
#16S rRNA feature table management
counts <- read.csv("16S_featuretable_paper.csv",header = TRUE, row.names = "ID")
counts=as.data.frame(counts)

#filtered data
#at least 10 reads in at least 5 samples 
filtered.counts <- counts[rowSums(counts>10)> 5, ] 
write.csv(as.data.frame(filtered.counts), 
          file="16S_filtered_featuretable_paper.csv")

#calculate relative abundance
data <- lapply(filtered.counts[], function(x) x/sum(x))
data= as.data.frame(data)
write.csv(as.data.frame(data), 
          file="10readsin5samples_16S_rel_abun.csv.csv")
